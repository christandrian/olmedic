<?php $this->start('store'); 
echo $data['data_store']['Nama'];
$this->end(); ?>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('.add').on('click', function(event) {
            ...
            t2.row.add(['', x.children().eq(1).text() + '<input type="hidden" value="' + x.children().find('.id_prod').val() + '" name="id_prod[]" />', '<input type="number" style="width:50px" max="' + x.children().eq(3).text() + '" min="1" name="qty[]" value="1" class="qtycart"/><small>' + x.children().find('.metric_prod').val() + '</small>', x.children().eq(4).text() + '<input type="hidden" value="' + x.children().eq(4).text() + '" name="price[]" />', x.children().eq(5).text() + '<input type="hidden" value="' + x.children().eq(5).text() + '" name="disc[]" />', '<input type="button" value="&times;" class="btn btn-default del"/>'
            ]).draw();
